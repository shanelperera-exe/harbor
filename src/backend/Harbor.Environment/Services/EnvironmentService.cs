...
public class EnvironmentService(IEnvironmentRepository environmentRepository) : IEnvironmentService
{
    ...
    public async Task<(bool Success, string? Error, bool Forbidden, EnvironmentResponse? Data)> CreateAsync(string projectId, CreateEnvironmentRequest request, int userId, bool isAdmin)
    {
        ...
        if (await environmentRepository.TypeExistsForProjectAsync(access.Id, type)) return (false, $"A {type} environment already exists for this project.", false, null);

        var environment = new EnvironmentEntity { PublicId = Harbor.Common.Utilities.IdGenerator.EnvironmentId(), ProjectId = access.Id, Name = name, Type = type, CreatedAt = DateTime.UtcNow };
        environment.Id = await environmentRepository.CreateAsync(environment);
        return (true, null, false, ToResponse(environment));
    }

    public async Task<(bool Success, string? Error, bool Forbidden, List<EnvironmentResponse>? Data)> GetByProjectAsync(string projectId, int userId, bool isAdmin)
    {
        var access = await environmentRepository.GetProjectAccessAsync(projectId);
        if (!access.Exists || access.IsArchived) return (false, "Project not found or archived.", false, null);
        if (!isAdmin && access.OwnerId != userId) return (false, "You do not have permission to view this project's environments.", true, null);
        return (true, null, false, (await environmentRepository.GetByProjectIdAsync(access.Id)).Select(ToResponse).ToList());
    }

    public async Task<(bool Success, string? Error, bool Forbidden, EnvironmentResponse? Data)> UpdateAsync(string projectId, int environmentId, UpdateEnvironmentRequest request, int userId, bool isAdmin)
    {
        ...
        var type = SupportedTypes.FirstOrDefault(t => string.Equals(t, request.Type?.Trim(), StringComparison.OrdinalIgnoreCase));
        if (type is null) return (false, "Environment type must be Development, Staging, or Production.", false, null);
        if (!string.Equals(name, environment.Name, StringComparison.Ordinal) && await environmentRepository.HasDeploymentHistoryAsync(access.ProjectId, environment.Name))
        ...
        if (await environmentRepository.TypeExistsForProjectAsync(access.ProjectId, type, environmentId)) return (false, $"A {type} environment already exists for this project.", false, null);
    ...
    public async Task<(bool Success, string? Error, bool Forbidden, EnvironmentRemovalResponse? Data)> RemoveAsync(string projectId, int environmentId, int userId, bool isAdmin)
    {
        var access = await AuthorizeManagementAsync(projectId, userId, isAdmin);
        if (!access.Success) return (false, access.Error, access.Forbidden, null);
        ...
        var environment = await environmentRepository.GetByIdAsync(environmentId, access.ProjectId);
        if (environment is null || !environment.IsActive) return (false, "Environment not found or inactive.", false, null);
        if (await environmentRepository.HasDeploymentHistoryAsync(access.ProjectId, environment.Name))
        {
            if (!await environmentRepository.DeactivateAsync(environmentId, access.ProjectId, DateTime.UtcNow)) return (false, "Environment could not be deactivated.", false, null);
        ...
        if (!await environmentRepository.DeleteAsync(environmentId, access.ProjectId)) return (false, "Environment could not be removed.", false, null);
        return (true, null, false, new EnvironmentRemovalResponse { Deactivated = false, Message = "Environment was removed." });
    }

    private async Task<(bool Success, string? Error, bool Forbidden, int ProjectId)> AuthorizeManagementAsync(string projectId, int userId, bool isAdmin)
    {
        var access = await environmentRepository.GetProjectAccessAsync(projectId);
    ...
    {
        Id = environment.Id, PublicId = string.IsNullOrEmpty(environment.PublicId) ? environment.Id.ToString() : environment.PublicId, ProjectId = environment.ProjectId, Name = environment.Name,
